...
## 2024-06-25 - Particle Hit-Testing Overhead
**Learning:** Having hundreds of animating, layered DOM elements (like the `.star` particles) forces the browser to continually perform hit-testing against them during scroll and mouse movements. This causes unnecessary Main Thread CPU overhead and potential jank, especially on lower-end devices.
**Action:** Always add `pointer-events: none;` to purely decorative background particles or elements that do not require user interaction to eliminate hit-testing overhead.
